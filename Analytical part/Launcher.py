...
def search_b(x: list, y: list) -> float:
    """
    Поиск коэффициента b по методу МНК.
...
def show_plot(m_y, m_x, n_y, n_x):
    """Строит графики для всех переданных данных.
    """
    data = [[n_y, n_x], [m_y, m_x]]
    for i in range(2):
        fig = plt.figure()
...
def main():
    """Функция исполнения основного тела программы.
    """
    try:
        ...
        d: float = 0.0012  # Диаметр контакта
        ...
        show_plot(i_out_f, v_out_f, i_out_r, v_out_r)
